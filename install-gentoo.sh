...
PASSWORD_FILE=/tmp/gentoo_passwords
...
messaggio "Tipo di installazione"
read -p "Installazione desktop? (con NetworkManager, wpa_supplicant) [S/n]: " DESKTOP_INSTALL
DESKTOP_INSTALL=${DESKTOP_INSTALL:-S}
...
# Salva password in file temporaneo sicuro per il chroot
cat > "$PASSWORD_FILE" <<PWDEOF
root:$ROOT_PASSWORD
...
# Partizione EFI: start=1MiB, end=EFI_MIB+1 → size esattamente EFI_MIB
END_EFI=$((EFI_MIB + 1))
parted -s "$DISK" mkpart primary fat32 1MiB "${END_EFI}MiB"
parted -s "$DISK" set 1 esp on

# Partizione swap: start=END_EFI, end=END_EFI+SWAP_MIB → size esattamente SWAP_MIB
...
# Verifica SHA512 robusta: calcola il checksum del file e lo cerca nel .DIGESTS.asc
SHA512_LOCAL=$(sha512sum "$STAGE_BASENAME" | awk '{print $1}')
SHA512_EXPECTED=$(grep -A1 SHA512 "$STAGE_FILE".DIGESTS.asc | grep -v '^--' | grep "$STAGE_BASENAME" | awk '{print $1}')
if [ "$SHA512_LOCAL" = "$SHA512_EXPECTED" ]; then
    echo "Verifica integrità stage3: OK"
else
    echo "ERRORE: verifica SHA512 fallita per $STAGE_BASENAME"
    exit 1
fi
...
if lspci 2>/dev/null | grep -qi 'vga.*intel'; then
    DETECTED_VIDEO="intel"
elif lspci 2>/dev/null | grep -qi 'vga.*nvidia'; then
    DETECTED_VIDEO="nvidia"
elif lspci 2>/dev/null | grep -qi 'vga.*amd\|vga.*ati\|vga.*radeon'; then
    DETECTED_VIDEO="amdgpu radeonsi"
fi

# ────────────────────────────── PREPARAZIONE CHROOT ──────────────────────────────
cp --dereference /etc/resolv.conf /mnt/gentoo/etc/
cp "$PASSWORD_FILE" /mnt/gentoo/tmp/gentoo_passwords
chmod 600 /mnt/gentoo/tmp/gentoo_passwords
...
echo "CPU_FLAGS_X86=\"\$(cpuid2cpuflags 2>/dev/null | sed 's/CPU_FLAGS_X86: //')\"" >> /etc/portage/make.conf

# ── Profilo ──
BEST_PROFILE=\$(eselect profile list 2>/dev/null | grep 'desktop' | grep -v 'experimental\|systemd' | tail -1 | awk '{print \$2}' | tr -d '[]')
if [ -n "\$BEST_PROFILE" ]; then
    eselect profile set "\$BEST_PROFILE"
else
    FALLBACK=\$(eselect profile list 2>/dev/null | grep -v 'experimental\|systemd' | tail -1 | awk '{print \$2}' | tr -d '[]')
    [ -n "\$FALLBACK" ] && eselect profile set "\$FALLBACK" || echo "ATTENZIONE: seleziona manualmente un profilo con 'eselect profile list'"
fi
...
read -p "Installare sys-firmware/sof-firmware? (solo Intel con audio DSP) [s/N]: " INSTALL_SOF
if [[ "\${INSTALL_SOF^^}" == "S" ]]; then
    emerge --quiet sys-firmware/sof-firmware
fi

# ── Video drivers ──
if [ -n "$DETECTED_VIDEO" ]; then
    echo "VIDEO_CARDS=\"$DETECTED_VIDEO\"" >> /etc/portage/make.conf
fi
...
# ── Password da file temporaneo ──
while IFS=: read -r user pass; do
    echo "\$user:\$pass" | chpasswd
done < /tmp/gentoo_passwords
rm -f /tmp/gentoo_passwords
...
# ── Rete ──
if [[ "${DESKTOP_INSTALL^^}" == "S" ]]; then
    emerge --quiet net-misc/networkmanager net-wireless/wpa_supplicant
